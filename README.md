...
- `value`: `{value}` / `{model}` = full model id or `empty_text`, `{short_model}` = model id after the final `/`, `{provider}` = current model provider (e.g. `anthropic`, `openai`, `openrouter`), `{thinking}` = current thinking level.
